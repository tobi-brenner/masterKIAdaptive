...
export const fetchUsers = async (query = ""): Promise<User[]> => {
  ...
  try {
    ...
    const res = await fetch(`http://localhost:8080/users?query=${encodeURIComponent(query)}`);
...
export const fetchEnrolledUsers = async (courseId: string):Promise<User[]> => {
  noStore();
  try {
    const response = await fetch(`http://localhost:8080/users/enrolled?courseId=${courseId}`);
    if (!response.ok) {
